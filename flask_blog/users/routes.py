...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flash('Logged In. You are now logged in.', 'success')
...
def account():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Account Updated. Your account has been updated!', 'success')
...
def user_posts(email):
    ...
    return render_template('user_posts.html', posts=posts, user=user ,title='Post for {}'.format(user.email))
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Email Sent. Follow instructions to reset your password', 'info')
        return redirect( url_for('users.login'))
...
def reset_token(token):
    ...
    if not user:
        flash('Invalid Token. That is an invalid or expired token', 'warning')
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Password Updated. You are now able to log in', 'info')
        return redirect( url_for('users.login'))
